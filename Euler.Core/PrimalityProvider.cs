...
namespace Euler.Core
{
    public class PrimalityProvider : IEnumerable<long>
    {
        ...
        /// <summary>
        /// Returns the index of the first prime above the max.
        /// </summary>
        /// <param name="max"></param>
        /// <returns></returns>
        public int FirstIndexAbove(long max)
        {
            int result = 1;
        ...
        public string WriteNumber(long input)
        {
            var factors = Decompose(input);
        ...
        internal Dictionary<long, long> Decompose(long candidate)
        {
            ...
            while (buffer > 1)
            {
                var primeFactor = primeSet[primIdx];
        ...
        internal PrimeDecomposition Decompose(long candidate, long power)
        {
            ...
            while (buffer > 1)
            {
                ...
                if (primePower > 0)
                {
                    result[primeFactor] = primePower * power;
                }
