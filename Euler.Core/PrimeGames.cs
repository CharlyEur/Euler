...
namespace Euler.Core
{
    public class PrimeGames
    {
        ...
        internal static long FindLongestQuadratics(int coefficientBound)
        {
            var max = (int) Math.Pow(10, (int) 2 * Math.Log10(coefficientBound) + 1);

            var gamer = new PrimeGames { Source = new PrimalityProvider(max) };

            return gamer.FindQuadratics(coefficientBound);
        }

        private long FindQuadratics(int coefficientBound)
        {
            int length = 0;
            long product = 1;

            int jLimit = (int) Source.FirstIndexAbove(coefficientBound);

            for (int i = -coefficientBound + 1; i <= coefficientBound; i++)
            {
                for (int j = 0; j <= jLimit; j++)
                {
                    var candidate = PrimeChainLength(i, Source[j]);
        ...
        private int PrimeChainLength(int i, long v)
        {
            var x = 1;
            var p_n = v;

            while (Source.IsPrime(Math.Abs(p_n)))
            {
                p_n = x * x + i * x + v;
                x++;
            }
