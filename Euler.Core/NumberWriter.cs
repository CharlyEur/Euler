...
using System.Linq;
using System.Text;
using QuickSum = System.Collections.Generic.Dictionary<int, int>;
...
namespace Euler.Core
{
    public class NumberWriter
    {
		...
		internal static long HowToWrite(int number)
		{
			var counter = 0;

			RecursiveHowToWrite(number, 0, number, ref counter);

			return counter;
		}

		internal static object WriteWithPrimesAbove(int maxLimit)
		{
			int count = 0;
			int candidate = 10; // as problem tells us 10 can be written in 5 different manners

			var provider = new PrimalityProvider(maxLimit);
			var adder = new RecursiveAdder(provider);

			while (count < maxLimit)
			{
				candidate++;
				count = adder.HowToWriteWithPrimes(candidate);
			}

			return candidate;
		}

		internal static void RecursiveHowToWrite(int number, int currentBuffer, int unit, ref int counter)
		{
			if (unit == 1)
			{
				counter++;
			...
			int newUnit = unit - 1;
			for (int i = 0; i <= number/newUnit; i++)
			{
				int newBuffer = currentBuffer + i * newUnit;
				if (newBuffer > number)
					break;
				...
				if (newUnit != 1)
					RecursiveHowToWrite(number, newBuffer, newUnit, ref counter);
			}
		}
